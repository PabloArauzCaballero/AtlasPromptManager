...
- Entorno estable (staging) con datos realistas y **sintéticos** (nunca datos reales de clientes;
  ver `data-privacy-sensitive`, `synthetic-test-data-generation`).
...
- Incluí roles reales: quién hace qué (analista, operador de back-office, comercio, cliente) y qué debería o no poder.
...
- **Aprobador**: quien tiene autoridad para aceptar (dueño de producto / responsable funcional; en
  riesgo o contabilidad, quien responde por esa área). Definí quién es antes de empezar.
...
- [ ] Aprobador definido y con autoridad (el dueño del área si aplica).
